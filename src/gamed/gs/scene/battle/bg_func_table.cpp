...
#define CHECK_WORLD_ID(id) \
	...
	if (!IS_BG_MAP(tmpxid)) \
	{ \
		LOG_WARN << "战场脚本调用错误！world64_id输入不正确:" << id; \
		return RC_FAILURE; \
	}
...
/// function table
///
luabind::CFunction bg_script_func_table[] = {

	// 地图元素
	{ "ActivateMapElem",      gamed::BGScript::ActivateMapElem      },
	{ "DeactivateMapElem",    gamed::BGScript::DeactivateMapElem    },

	// 任务
	{ "DeliverTask",          gamed::BGScript::DeliverTask          },
	{ "DeliverTaskToAll",     gamed::BGScript::DeliverTaskToAll     },

	// 计数器
	{ "IncreaseCounter",      gamed::BGScript::IncreaseCounter      },	
	{ "DecreaseCounter",      gamed::BGScript::DecreaseCounter      },
	{ "AssignmentCounter",    gamed::BGScript::AssignmentCounter    },
	{ "GetCounter",           gamed::BGScript::GetCounter           },
    { "LockCounter",          gamed::BGScript::LockCounter          },
	{ "UnlockCounter",        gamed::BGScript::UnlockCounter        },
	{ "IsCounterLocked",      gamed::BGScript::IsCounterLocked      },

	// 计时器
	{ "ActivateClock",        gamed::BGScript::ActivateClock        },
	{ "DeactivateClock",      gamed::BGScript::DeactivateClock      },

	// 关闭战场
	{ "CloseBattleGround",    gamed::BGScript::CloseBattleGround    },

    // 地图提示信息
    { "ShowPromptMessage",    gamed::BGScript::ShowPromptMessage    },
    { "ShowCountDown",        gamed::BGScript::ShowCountDown        },

    // 定点地图元素瞬移
    { "SpotMapElemTeleport",  gamed::BGScript::SpotMapElemTeleport  },

    // 定点怪移动到某点
    { "SpotMonsterMoveTo",    gamed::BGScript::SpotMonsterMoveTo    },
    { "SetSpotMonsterSpeed",  gamed::BGScript::SetSpotMonsterSpeed  },

};

int func_table_size = sizeof(BGScript::bg_script_func_table)/sizeof(luabind::CFunction);
...
int ActivateMapElem(lua_State* state)
{
	...
	{
		LOG_WARN << "战场脚本调用failure: ActivateMapElem()";
		return RC_FAILURE;
	}
...
int DeactivateMapElem(lua_State* state)
{
	...
	{
		LOG_WARN << "战场脚本调用failure: DeactivateMapElem()";
		return RC_FAILURE;
	}
...
int DeliverTask(lua_State* state)
{
	...
	if (task_id > 0 && role_id > 0)
	{
		...
		if (!target.IsPlayer())
		{
			LOG_WARN << "战场脚本调用failure: DeliverTask() -- role_id invalid";
			return RC_FAILURE;
		}
	...
	{
		LOG_WARN << "战场脚本调用failure: DeliverTask()";
		return RC_FAILURE;
	}
...
int DeliverTaskToAll(lua_State* state)
{
	...
	{
		LOG_WARN << "战场脚本调用failure: DeliverTaskToAll()";
		return RC_FAILURE;
	}
...
int IncreaseCounter(lua_State* state)
{
	...
	{
		LOG_WARN << "战场脚本调用failure: IncreaseCounter()";
		return RC_FAILURE;
	}
...
int DecreaseCounter(lua_State* state)
{
	...
	{
		LOG_WARN << "战场脚本调用failure: DecreaseCounter()";
		return RC_FAILURE;
	}
...
int AssignmentCounter(lua_State* state)
{
	...
	{
		LOG_WARN << "战场脚本调用failure: AssignmentCounter()";
		return RC_FAILURE;
	}
...
int ActivateClock(lua_State* state)
{
	...
	{
		LOG_WARN << "战场脚本调用failure: ActivateClock()";
		return RC_FAILURE;
	}
...
int DeactivateClock(lua_State* state)
{
	...
	{
		LOG_WARN << "战场脚本调用failure: DeactivateClock()";
		return RC_FAILURE;
	}
...
{
	// param
    int32_t bg_result  = 0; // 1~4，表示阵营1到4那个阵营胜利
	int64_t world64_id = 0;

	// process
	luabind::LuaEngine engine;
	engine.Init(state);
	CHECK_VALUE(engine.PopValue(bg_result));
	CHECK_VALUE(engine.PopValue(world64_id));

	CHECK_WORLD_ID(world64_id);
    if (bg_result < 1 || bg_result > 4)
    {
        LOG_WARN << "CloseBattleGround阵营填写错误!";
    ...
    param.bg_result = bg_result;
	send_plane_msg(GS_PLANE_MSG_SYS_CLOSE_BG, target, &param, sizeof(param));
