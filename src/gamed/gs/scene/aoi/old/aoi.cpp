...
AOI::object* AOI::NewObject(ObjectID id, int mode, IDType type, const A2DVECTOR& last, const A2DVECTOR& pos, bool area_detect)
{
						   ...
						   bool area_detect)
{
	...
	object* obj = NewObject(id, mode, type, init_pos, init_pos, area_detect);
...
void AOI::EnterViewCB(AOI_Callback aoi_cb, const object* watcher, const object* marker, void* cb_data)
{
	...
	if (it == watcher_hotpair_savemap_[watcher->id].end())
	{
		...
		watcher_info.id   = watcher->id;
		watcher_info.type = watcher->type;
		marker_info.id    = marker->id;
		marker_info.type  = marker->type;
