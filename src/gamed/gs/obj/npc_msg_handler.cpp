...
int Npc::MessageHandler(const MSG& msg)
{
	switch (msg.message)
	{
        ...
        case GS_MSG_MONSTER_MOVE:
            {
                if (msg.source != world_xid())
                    return 0;

                HandleMonsterMove(msg);
            }
            break;

        case GS_MSG_MONSTER_SPEED:
            {
                if (msg.source != world_xid())
                    return 0;

                HandleMonsterSpeed(msg);
            }
            break;

		default:
		    if (Unit::MessageHandler(msg) != 0 && pimp_->OnMessageHandler(msg) != 0)
			{
